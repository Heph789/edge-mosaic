...
- **Podcasts & YouTube** resolve as RSS underneath a directory page: Apple Podcasts
  (`podcasts.apple.com/.../id<n>`) → real feed via the iTunes Lookup API; YouTube
  channel/handle → `feeds/videos.xml?channel_id=…`. Both force `kind='long'` (a media
  episode is real content regardless of blurb length). See [`slice-1-ingestion-spike.md`].
